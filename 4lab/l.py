...
t_recovery = []
t_work = []
T = [-np.log(random()) / L for i in range(3)]
t_work.append(T[0])
t_work.append(T[1])
t_work.append(T[2])
working_on = [0, 0, 0]
...
for step in range(len(t)):
    ...
    for rem in remove_from_queue:
        queue.remove(rem)
    n_t[step] = 1 if (is_on[0] == 1 or is_on[1] == 1 or is_on[2] == 1) else 0
    for k in range(len(t_work)):
        t_work_mid += t_work[k]
    for k in range(len(t_recovery)):
        t_recovery_mid += t_recovery[k]
    t_work_mid /= len(t_work)
    t_recovery_mid /= len(t_recovery)
    # K_mid = t_work_mid / (t_work_mid + t_recovery_mid)
    return n_t, K_mid